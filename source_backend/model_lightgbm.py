...
class LightGBMModels:
    ...
    def fit(self, X: Optional[pd.DataFrame] = None, y: Optional[pd.Series] = None):
        ...
        # TODO: Add actual LightGBM training logic here
        return self
    ...
    def _create_model(self, use_best_params: bool = False) -> LGBMRegressor:
        """
        Creates and configures the LightGBM model.
    ...
    def _get_best_params(self, X_train: pd.DataFrame, y_train: pd.Series) -> dict:
        ...
        # Get best parameters directly from optimizer;
        best_params = optimizer.optimize()
        
        return best_params

    def cross(self) -> Tuple[pd.DataFrame, pd.DataFrame]:
        """
        Finds optimal hyperparameters (if not already found) and evaluates model performance 
        on the last forecasting_horizon periods for each division.
        ...
        if all_predictions:
            ...
            predictions_df = pd.DataFrame(columns=['ds', 'unique_id', 'LGBMRegressor', 'y'])
    ...
    def predict_future(self) -> pd.DataFrame:
        ...
        for division in divisions:
            ...
            X_train = X_train[future_df.drop('ds', axis=1).columns]
    ...
    def last_window(self, df: pd.DataFrame) -> pd.DataFrame:
        """
        Select only the last cutoff as the final result. Window evaluation still needs to be implemented for LightGBM.
        """
