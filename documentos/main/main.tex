...
\chapter{Revisão Bibliográfica}
  Neste capítulo será feita uma análise dos fatores e contexto que levaram à crise das Enchentes de 2024 no estado do
  Rio Grande do Sul, com um destaque para o aumento de precipitação causado pelo aquecimento global e os efeitos do
  mesmo na América Latina. Será também explorado a vulnerabilidade intrinseca de centros urbanos e a necessidade elevada
  de prevenção e monitoramento para tais situações.
    \section{Mudanças climáticas e desastres naturais}
    \input{climate.tex}
    \section{Enchente de 2024}
    \input{flood.tex}
    \section{Análise geográfica da RMPOA}
    \input{geo.tex}

\chapter{Revisão dos Modelos}
  Nesta sessão exploraremos os modelos escolhidos na parte experimental do projeto. Os motivos de escolha dos mesmos serão
  desenvolvidos em cada respectivo capítulo. Contudo, um dos maiores critérios de escolha do mesmo se baseou em ter pelo
  menos um representante das principais áreas de predição de séries temporais baseadas em dados. Logo, se viu necessária a
  presença de um modelo estatístico clássico como base de comparação, modelos de Redes Neurais devido ao seus grandes usos
  e efetividades e, por último, um modelo baseado em árvores de decisão devido a sua intuitiva compreensão e
  permissividade em círculos de ciência de dados. 
    ...
    \input{nhits.tex}
\chapter{Revisão das Métricas}
    \lipsum[1] 
    \section{Root Mean Squared Error}
    \input{rmse.tex}
    \section{Mean Absolute Error}
    \input{mae.tex}
    \section{Nash-Sutcliffe Model Efficiency Coefficient}
    \input{nse.tex}

\chapter{Revisão das Técnicas}
    \lipsum[1] 
