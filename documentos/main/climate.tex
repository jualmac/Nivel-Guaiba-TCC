...
\subsection{Aumento de precipitação e enchentes}
...
redução de temperaturas mínimas e ondas de calor, sendo as mesmas definidas por Dunn et al. (2024) \cite{dunn} como 3 ou
mais dias acima com a temperatura acima do 90º percentil. Um evento climático cujo mudança se destaca entre esses é a
precipitação. Em essência, o aumento de temperatura leva à uma maior evaporação da água e também a maior capacidade
atmosférica de reter esse vapor de água, o que resulta em chuvas mais intensas e irregulares. Uma definição mais
completa pode ser dada por Seneviratne et al. (2021) no capítulo 11.1 do sexto relatório, segundo grupo, do IPCC (AR6
WGII): 
...
atmosférica aumenta aproximadamente de acordo com a relação de Clausius–Clapeyron (C-C), com um aumento de cerca de 7\%
para cada 1°C de aquecimento médio global da superfície. (...) Projeções de modelos climáticos mostram que o aumento do
vapor d'água resulta em elevações significativas nos extremos de precipitação em todas as regiões, com uma magnitude que
varia entre 4\% e 8\% por 1°C de aquecimento da superfície.

\textit{Seneviratne et al. (2021)\cite{ipcc_ar6_wg2_chap11}, traduzido pelo autor.}

\end{citacao}

Uma mudança na temperatura não implica necessariamente em um aumento na precipitação, mas sim em alterações nos seus
padrões. Os complexos mecanismos atmosféricos que regulam a precipitação são influenciados pelas mudanças graduais de
temperatura, podendo tanto aumentar quanto reduzir não apenas a quantidade, mas também a intensidade das chuvas. Essas
variações são específicas a cada região, dependendo das particularidades climáticas locais. Por exemplo, no Brasil, os
dois efeitos ocorrem de forma simultânea. Na região Nordeste, há uma diminuição na quantidade e intensidade das chuvas,
enquanto na região Sudeste, há uma intensificação das mesmas.  

Essa mudança na precipitação leva diretamente à um aumento na quantidade de secas e de enchentes. Pois, se a
distribuição de precipitações aumenta ou diminui, os limiares que levam à esses eventos são mais facilmente alcançados. 

% El ÑINO:
% https://wmo.int/media/news/global-temperature-record-streak-continues-climate-change-makes-heatwaves-more-extreme

% IPCC AR6 WGI - Chapter 8, FAQ 8.2: Floods are a natural and important part of the water cycle but they can also
% threaten lives and safety, disrupt human activities, and damage infrastructure. Most inland floods occur when rivers
% overtop their banks (fluvial flooding) or when intense rainfall causes water to build up and overflow locally (pluvial
% flooding). Flooding is also caused by coastal inundation by the sea, rapid seasonal melting of snow, and the
% accumulation of debris, such as vegetation or ice, that stops water from draining away.
...
\subsection{Vulnerabilidade Urbana à enchentes} % Talvez mudar o título;
...
% -> Vulnerabilidade dos que contribuem menos. Tentar ao máximo ficar no tópico de América Latina; -> Vulnerabilidade
% Urbana em si;